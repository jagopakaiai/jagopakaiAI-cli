...
const JagoPakaiAI_VERSION = readJagoPakaiAIVersion();
const JagoPakaiAI_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/JagoPakaiAI-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'JagoPakaiAI_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const JagoPakaiAI_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function readJagoPakaiAIVersion() {
...
function brandMarkup() {
  const version = escapeHtmlText(JagoPakaiAI_VERSION);
  const text = JagoPakaiAI_TELEMETRY_DISABLED
    ? 'Prime Radiant JagoPakaiAI v' + version
    : 'JagoPakaiAI v' + version;
  const logo = JagoPakaiAI_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + JagoPakaiAI_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(JagoPakaiAI_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="https://github.com/obra/JagoPakaiAI">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
